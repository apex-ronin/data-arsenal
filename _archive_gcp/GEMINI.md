...
## 🚨 CRITICAL: Agent Handoffs & Semantic Verification (M-26-04 Incident Post-Mortem)

**DO NOT REPEAT THE CLAUDE.MD FAILURE:**
1. **Commander's Intent is Mandatory:** Never write localized agent prompts (`CLAUDE.md`) that entirely strip the global roadmap context. A blind agent cannot evaluate qualitative constraints.
2. **The "Continuous Execution" Trap:** Never instruct a coding agent to "execute continuously as long as unit tests pass" when dealing with compliance pivots, deprecations, or semantic changes. Unit tests do not validate filenames, docstrings, or semantic intent—they only validate execution paths.
3. **Semantic Hard-Stops:** If a directive contains "HOLD", "DEPRECATED", or "PIVOT", the agent MUST HALT. It is forbidden to use the test suite as the sole metric for completion on these tasks. You must perform a full semantic audit (including file names, folder names, and docstrings) and await human approval before stamping "Done".
4. **Trust but Verify:** No substantive commit ships without peer-review and diff verification.
5. **Primary Source Inline:** Any regulatory citation (e.g., M-26-04) must include a primary source URL directly in a code comment.
6. **STATE.md Enforcement:** Appending an atomic log line to the `STATE.md` ground truth ledger is a strict requirement before closing any session.
